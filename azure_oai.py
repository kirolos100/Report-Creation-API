...
def get_insights(summaries):
    ...
    Key Insights: After the executive summary, present a ranked "Key Insights" section. For each insight produce a single paragraph that begins with a concise insight statement (two-sentences claim) followed by supporting evidence drawn from the call summaries. Each insight paragraph must clearly state: why the insight matters (business or customer impact), likely root causes or contributing factors, and a recommended corrective action.
    Top Issues & Ranking: Identify and rank the most common issues or complaints using a combined scoring approach that accounts for both frequency (how many distinct calls mention the issue) and order-of-appearance (earlier/larger clusters) (without writing any numbers, order and rank it only). For each top issue produce one paragraph that covers: a concise description of the issue, whether the issue shows a rising/declining/stable trend (if timestamps are available), a short anonymized example excerpt, likely root causes (process, training, product, technical), business or operational impact, recommended corrective actions, suggested owner/team for implementation, priority level (High/Medium/Low), and expected measurable benefit.
